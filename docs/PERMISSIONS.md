...
`IT_SUPPORT_TECHNICIAN` receives only `support.read` and `support.respond`; ticket RLS additionally scopes visibility to tickets assigned to the employee linked to `auth.uid()`. `SUPER_ADMIN` and `ADMIN` receive the `support.*` capabilities, including assignment, configuration and reporting. Customer roles receive only `customer.support.read/create/respond`, and customer ticket rows are checked through `private.customer_portal_has_access(customer_id)`. Internal support activity is separately guarded by `support.internal_note`; customer messages are always non-internal. Assignment, status transitions, and customer resolution confirmation are enforced by narrow authenticated RPCs, not direct table updates. The video provider button remains disabled until a provider is configured. Telegram linking and preferences are profile-scoped; bot administration requires `settings.manage`, internal staff conversations require `chat.manage`, support ticket replies require the assigned `support.respond` scope or `support.view_all`, and customer conversations require a matching active customer portal access record. Internal notes are not bridged. The service-role Edge Function independently verifies signed Betanor sessions for user actions and a Vault-held transport secret for Telegram webhooks/dispatch.
