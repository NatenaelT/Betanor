...
Tickets carry customer, optional existing commercial support contract/project, customer requester, and an optional assigned employee. Support conversations extend `chat_conversations` via `support_ticket_id`; messages, presence, private chat attachments and customer portal access remain in the existing chat system. Support tasks link to canonical `tasks`; documents are linked through existing `document_links`; in-app notifications use `notifications`, while email/Telegram requests are queued in a support outbox. Migration `202609240001_telegram_chat_notifications.sql` adds the per-user Telegram link, update deduplication, retryable outbox delivery, protected linking/configuration RPCs, and a Vault-backed asynchronous dispatcher. `202609240002_secure_telegram_rpc_surface.sql` restricts challenge and administrative RPCs to the service-role Edge Function, while `202609240003_fix_telegram_admin_access_check.sql` evaluates the JWT-verified profile's existing RBAC grants rather than relying on `auth.uid()` inside a service-role call. `202609240004_telegram_all_application_notifications.sql` fans out opted-in, non-chat/support application notifications to the same outbox without copying notification titles or bodies to Telegram. `telegram_connections` references the existing `profiles` identity and can select an existing `chat_conversations` row; it does not create a new identity or conversation model. `telegram_link_challenges` stores only a SHA-256 hash of a short-lived linking token, and `telegram_processed_updates` prevents Telegram webhook retries from inserting duplicate canonical chat messages. `support_sessions` stores session metadata only: it does not create a video meeting or persist provider secrets. Lifecycle records describe customer IT joiner/leaver work and do not duplicate HR employee onboarding.
...
+
+PROFILE → EMAIL_MESSAGE → EMAIL_MESSAGE_LINK → LETTER / PROJECT / TASK / TENDER / QUOTATION / CONTRACT / RFQ / CUSTOMER / SUPPORT_TICKET / EMPLOYEE
+EMAIL_MESSAGE → EMAIL_MESSAGE (reply/follow-up parent)
+TENDER → TENDER_REQUIREMENT → optional private-storage attachment
+
