...
function tone(status: string) {
  return status === "approved" ? "success" as const : status === "rejected" ? "danger" as const : status === "in_review" ? "info" as const : "draft" as const;
}
...
async function requestLeave(data: FormData) {
  ...
  const leaveTypeId = String(data.get("leaveTypeId") ?? "");
  const duration = startsOn && endsOn ? workdays(startsOn, endsOn) : 0;
  if (!startsOn || !endsOn || !leaveTypeId || duration < 1) return;
...
async function decideLeave(data: FormData) {
  ...
  const userId = claims?.claims.sub;
  if (!userId) return;
  const { data: actor } = await supabase.from("employees").select("id").eq("profile_id", userId).maybeSingle();
  if (!actor?.id) return;
  const { data: request } = await supabase.from("leave_requests").select("id,status,manager_approved_at,employees!leave_requests_employee_id_fkey(manager_id)").eq("id", requestId).maybeSingle();
  if (!request) return;
  const targetEmployee = Array.isArray(request.employees) ? request.employees[0] : request.employees;
...
export default async function LeavePage() {
  ...
  const [{ data: ownEmployee }, { data: leaveTypes }, { data: requests, error }, { data: assignments }] = await Promise.all([
    userId ? supabase.from("employees").select("id,first_name,last_name,employee_number,employment_contracts(salary_amount,currency_code,status,starts_on,ends_on)").eq("profile_id", userId).maybeSingle() : Promise.resolve({ data: null }),
    workspaceId ? supabase.from("leave_types").select("id,name,is_paid,annual_allowance").eq("workspace_id", workspaceId).order("name") : Promise.resolve({ data: [] as never[] }),
    supabase.from("leave_requests").select("id,employee_id,starts_on,ends_on,duration_days,reason,status,manager_approved_at,hr_approved_at,employees!leave_requests_employee_id_fkey(id,first_name,last_name,employee_number,manager_id),leave_types(name,is_paid,annual_allowance)").order("created_at", { ascending: false }),
    userId ? supabase.from("user_roles").select("roles(code)").eq("user_id", userId) : Promise.resolve({ data: [] as never[] }),
  ]);

  const roleCodes = (assignments ?? []).map((x) => x.roles?.[0]?.code).filter(Boolean);
  const hrApprover = roleCodes.some((code) => ["SUPER_ADMIN", "ADMIN", "HR_MANAGER"].includes(code));
