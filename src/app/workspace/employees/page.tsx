...
export default async function EmployeesPage() {
  ...
  return <main className="mx-auto max-w-7xl px-6 py-10 lg:px-8">
    ...
    {workspaceId ? <Card className="mt-8 p-6"><h2 className="text-lg font-semibold text-[var(--betanor-navy)]">Create employee profile</h2><p className="mt-1 text-sm text-[var(--betanor-muted)]">The employee number is generated by Supabase. Link an existing staff profile to enable self-service leave and salary visibility.</p><form action={createEmployee} className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      <div><FieldLabel htmlFor="employee-first">First name</FieldLabel><Input id="employee-first" name="firstName" required/></div>
      <div><FieldLabel htmlFor="employee-last">Last name</FieldLabel><Input id="employee-last" name="lastName" required/></div>
      <div><FieldLabel htmlFor="employee-profile">Portal profile</FieldLabel><select id="employee-profile" name="profileId" className="min-h-10 w-full rounded-lg border border-[var(--betanor-border)] bg-white px-3 text-sm"><option value="">Link later</option>{profiles.data?.map((item) => <option key={item.id} value={item.id}>{item.full_name || "Unnamed profile"}{item.job_title ? ` · ${item.job_title}` : ""}</option>)}</select></div>
      <div><FieldLabel htmlFor="employee-email">Work email</FieldLabel><Input id="employee-email" name="workEmail" type="email"/></div>
      <div><FieldLabel htmlFor="employee-phone">Work phone</FieldLabel><Input id="employee-phone" name="workPhone" type="tel"/></div>
      <div><FieldLabel htmlFor="employee-start">Start date</FieldLabel><Input id="employee-start" name="hireDate" required type="date"/></div>
      <div><FieldLabel htmlFor="employee-probation">Probation end date</FieldLabel><Input id="employee-probation" name="probationEndDate" type="date"/></div>
      <div><FieldLabel htmlFor="employee-department">Department</FieldLabel><select id="employee-department" name="departmentId" className="min-h-10 w-full rounded-lg border border-[var(--betanor-border)] bg-white px-3 text-sm"><option value="">Select department</option>{departments.data?.map((department) => <option key={department.id} value={department.id}>{department.name} · {department.code}</option>)}</select></div>
      <div><FieldLabel htmlFor="employee-position">Position</FieldLabel><select id="employee-position" name="positionId" className="min-h-10 w-full rounded-lg border border-[var(--betanor-border)] bg-white px-3 text-sm"><option value="">Select position</option>{positions.data?.map((position) => <option key={position.id} value={position.id}>{position.title}{position.code ? ` · ${position.code}` : ""}</option>)}</select></div>
      <div><FieldLabel htmlFor="employee-manager">Immediate manager</FieldLabel><select id="employee-manager" name="managerId" className="min-h-10 w-full rounded-lg border border-[var(--betanor-border)] bg-white px-3 text-sm"><option value="">No manager recorded</option>{rows.map((employee) => <option key={employee.id} value={employee.id}>{employee.first_name} {employee.last_name} · {employee.employee_number}</option>)}</select></div>
      <div><FieldLabel htmlFor="employee-type">Employment type</FieldLabel><select id="employee-type" name="employmentType" defaultValue="full_time" className="min-h-10 w-full rounded-lg border border-[var(--betanor-border)] bg-white px-3 text-sm"><option value="full_time">Full time</option><option value="part_time">Part time</option><option value="contract">Contract</option></select></div>
      <div><FieldLabel htmlFor="employee-salary">Monthly salary (ETB)</FieldLabel><Input id="employee-salary" min="0" name="salary" step="0.01" type="number"/></div>
      <div><FieldLabel htmlFor="employee-contract">Contract title</FieldLabel><Input id="employee-contract" name="contractTitle" defaultValue="Employment contract"/></div>
      <div className="lg:col-span-3 flex flex-wrap items-center gap-3"><span className="rounded-full bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-900">Standard schedule: Monday–Friday · 8 hours/day · 40 hours/week</span><Button type="submit">Create employee profile</Button></div>
    </form></Card> : <Card className="mt-8 p-6">Employee access is required for this workspace.</Card>}
