...
async function uploadProductImage(supabase: Awaited<ReturnType<typeof createClient>>, fileValue: FormDataEntryValue | null, workspaceId: string) {
  if (!(fileValue instanceof File) || fileValue.size === 0 || !fileValue.type.startsWith("image/") || fileValue.size > 20 * 1024 * 1024) return null;
  const safeName = fileValue.name.replace(/[^A-Za-z0-9._-]/g, "_").slice(-100) || "product-image";
  const path = `${workspaceId}/catalogue/${randomUUID()}-${safeName}`;
...
async function createProduct(formData: FormData) {
  ...
  const imagePath = await uploadProductImage(supabase, formData.get("image"), access.workspaceId);
...
async function updateCatalogueItem(data: FormData) {
  "use server";
  const table = String(data.get("table") ?? ""); const id = String(data.get("id") ?? "").trim(); const name = String(data.get("name") ?? "").trim(); const slug = String(data.get("slug") ?? "").trim().toLowerCase();
  if (!id || !name || !slug || !["product_categories", "products"].includes(table)) return;
  const supabase = await createClient();
  if (table === "product_categories") await supabase.from("product_categories").update({ name, slug, description: String(data.get("description") ?? "").trim() || null }).eq("id", id);
  if (table === "products") await supabase.from("products").update({ name, slug, brand: String(data.get("brand") ?? "").trim() || null, model: String(data.get("model") ?? "").trim() || null, short_description: String(data.get("description") ?? "").trim() || null }).eq("id", id);
  revalidatePath("/workspace/products"); revalidatePath("/products");
}
...
export default async function ProductCataloguePage() {
  ...
  const [categories, products] = await Promise.all([
    ...
    supabase.from("products").select("id, name, slug, sku, brand, model, main_image_path, status, published_at, product_categories(name)").order("updated_at", { ascending: false }),
  ]);
...
function CatalogueCollection({ label, rows, table }: { label: string; rows: Array<{ id: string; name: string; slug: string; status: string; published_at: string | null; sku?: string | null; brand?: string | null; model?: string | null; main_image_path?: string | null; product_categories?: Array<{ name: string }> | null }>; table: "product_categories" | "products" }) {
  return <Card className="overflow-hidden"><div className="flex items-center justify-between border-b border-[var(--betanor-border)] px-5 py-4"><h2 className="font-semibold text-[var(--betanor-navy)]">{label}</h2><span className="text-sm text-[var(--betanor-muted)]">{rows.length}</span></div>{rows.length ? <ul className="divide-y divide-[var(--betanor-border)]">{rows.map((row) => <li key={row.id} className="px-5 py-4"><div className="flex items-center justify-between gap-3"><div className="min-w-0">{row.main_image_path ? <img src={`${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/betanor-site-assets/${row.main_image_path}`} alt="" className="mb-2 h-12 w-16 rounded object-cover" /> : null}<p className="truncate text-sm font-semibold text-[var(--betanor-navy)]">{row.name}</p><p className="mt-1 truncate text-xs text-[var(--betanor-muted)]">{row.brand ? `${row.brand}${row.model ? ` · ${row.model}` : ""} · ` : ""}/{row.slug}{row.product_categories?.[0]?.name ? ` · ${row.product_categories[0].name}` : ""}</p></div><div className="flex shrink-0 items-center gap-2"><Badge tone={row.status === "active" ? "success" : "draft"}>{row.status}</Badge><details className="relative"><summary className="cursor-pointer list-none rounded-lg border border-[var(--betanor-border)] px-3 py-1.5 text-xs font-semibold text-[var(--betanor-navy)] hover:border-[var(--betanor-blue)]">Edit</summary><form action={updateCatalogueItem} className="absolute top-9 right-0 z-10 grid w-64 gap-2 rounded-xl border border-[var(--betanor-border)] bg-white p-3 shadow-xl"><input type="hidden" name="table" value={table}/><input type="hidden" name="id" value={row.id}/><Input name="name" defaultValue={row.name}/><Input name="slug" defaultValue={row.slug}/>{table === "products" ? <><Input name="brand" defaultValue={row.brand ?? ""} placeholder="Brand"/><Input name="model" defaultValue={row.model ?? ""} placeholder="Model"/></> : null}<Input name="description" placeholder="Description"/><Button type="submit" size="sm">Save changes</Button></form></details>{row.status !== "active" ? <form action={publishCatalogueItem.bind(null, table, row.id)}><Button type="submit" size="sm">Publish</Button></form> : null}{row.status !== "archived" ? <form action={archiveCatalogueItem.bind(null, table, row.id)}><Button type="submit" size="sm" variant="outline">Archive</Button></form> : null}</div></div></li>)}</ul> : <p className="px-5 py-8 text-sm text-[var(--betanor-muted)]">No {label.toLowerCase()} have been created.</p>}</Card>;
}
