...
import { TelegramProfileSettings } from "@/components/telegram/telegram-profile-settings";
...
export default async function ProfilePage() {
  ...
  const [{ data: profile }, { data: telegramConnection }, { data: telegramPreference }] = await Promise.all([
    supabase.from("profiles").select("full_name,job_title,avatar_path,email_address,account_type,is_active").eq("id", access.userId).maybeSingle(),
    supabase.from("telegram_connections").select("telegram_username,linked_at").eq("profile_id", access.userId).maybeSingle(),
    supabase.from("support_notification_preferences").select("telegram").eq("profile_id", access.userId).maybeSingle(),
  ]);
