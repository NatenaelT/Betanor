...
export default async function TaskDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="mx-auto max-w-6xl px-5 py-8 sm:px-6 lg:px-8 lg:py-10">
    ...
    <div className="mt-5 flex flex-col justify-between gap-4 md:flex-row md:items-start"><div><div className="flex flex-wrap items-center gap-2"><Badge tone={tone(task.status)}>{titleCase(task.status)}</Badge><Badge tone={task.priority === "urgent" ? "danger" : task.priority === "high" ? "warning" : "neutral"}>{titleCase(task.priority)} priority</Badge></div><h1 className="mt-3 text-3xl font-semibold text-[var(--betanor-navy)]">{task.title}</h1><p className="mt-2 text-sm text-[var(--betanor-muted)]">{task.task_code} · {project ? `${project.project_code} · ${project.name}` : "Internal work"}{milestone ? ` · ${milestone.title}` : ""}</p></div><div className="flex items-center gap-4"><div className="text-sm text-[var(--betanor-muted)]">Created {new Date(task.created_at).toLocaleDateString("en-GB")}</div>{access.permissions.has("email.send") ? <Link href={"/workspace/emails/new?" + new URLSearchParams({ relatedModule: "tasks", relatedId: task.id, relatedLabel: task.task_code + " · " + task.title, subject: "Task update: " + task.title }).toString()} className="text-sm font-semibold text-[var(--betanor-blue)] hover:underline">Email task team</Link> : null}</div></div>
