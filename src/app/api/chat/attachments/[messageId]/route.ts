...
export async function GET(_request: Request, { params }: { params: Promise<{ messageId: string }> }) {
  ...
  const { data: message, error } = await supabase
    .from("chat_messages")
    .select("attachment_path,attachment_name,task_id,sender_kind,chat_conversations(id,workspace_id,customer_id)")
    .eq("id", messageId)
    .maybeSingle();
  ...
  const canReadInternalGroup = Boolean(
    message.sender_kind === "telegram_group" && access.hasStaffRole &&
    access.permissions.has("chat.internal.read") &&
    conversation?.workspace_id && access.workspaceId === conversation.workspace_id,
  );
  let canReadAssignedTaskGroup = false;
  if (message.sender_kind === "telegram_group" && message.task_id && access.hasStaffRole && access.workspaceId === conversation?.workspace_id) {
    const { data: task } = await supabase.from("tasks").select("id").eq("id", message.task_id).maybeSingle();
    canReadAssignedTaskGroup = Boolean(task?.id);
  }
  ...
  if (!canReadChat && !canReadInternalGroup && !canReadSupport && !canReadAssignedTaskGroup) {
