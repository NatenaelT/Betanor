...
export default async function ServicesPage() {
  ...
  const [{ data: services }, { data: products }, { data: industries }, { data: caseStudies }] = await Promise.all([
    supabase.from("services").select("id,title,slug,excerpt,content").eq("status", "active").not("published_at", "is", null).order("title"),
    supabase.from("products").select("id,name,slug,brand,model,short_description,availability,warranty,product_categories(name)").eq("status", "active").not("published_at", "is", null).order("name"),
