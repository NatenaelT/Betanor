...
export function UserManagementPanel({ roles, permissions, rolePermissions }: Props) {
  ...
  function editUser(user: UserRecord) {
    ...
    setEditingId(user.id); setAccountType(type); setRoleCode(nextRole); setFullName(user.profile?.full_name ?? ""); setEmail(user.email ?? ""); setPhone(user.profile?.phone_e164 ?? user.phone ?? ""); setJobTitle(user.profile?.job_title ?? ""); setPassword(""); setPermissionOverrides(nextOverrides); setIsActive(user.profile?.is_active !== false); setNotice({ tone: "info", text: `Editing ${user.email || user.id}` }); window.scrollTo({ top: 0, behavior: "smooth" });
  }
