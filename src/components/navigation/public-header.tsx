...
export function PublicHeader() {
  ...
  const links = [["Services", "/services"], ["Products", "/products"], ["Solutions", "/projects"], ["Industries", "/industries"], ["Insights", "/insights"], ["Company", "/about"]] as const;

  return (
    <header className="relative border-b border-[var(--betanor-border)] bg-white">
      <nav className="mx-auto flex min-h-20 max-w-7xl items-center justify-between gap-6 px-6 lg:px-8" aria-label="Primary navigation">
        ...
        <div className="flex items-center gap-3"><Link className="hidden rounded-lg border border-[var(--betanor-border)] px-4 py-2 text-sm font-semibold text-[var(--betanor-navy)] sm:inline-flex" href="/contact">Request consultation</Link><Link className="rounded-lg bg-[var(--betanor-navy)] px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-[var(--betanor-blue)]" href="/contact#quote">Request a quote</Link><button aria-expanded={isOpen} aria-label="Open site navigation" className="grid size-10 place-items-center rounded-lg text-[var(--betanor-navy)] hover:bg-slate-100 lg:hidden" onClick={() => setIsOpen(!isOpen)}>☰</button></div>
      </nav>
