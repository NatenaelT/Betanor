...
export function WorkspaceTopbar({ email }: { email: string }) {
  ...
  return <header className="sticky top-0 z-20 flex min-h-16 items-center gap-3 border-b border-[var(--betanor-border)] bg-[var(--betanor-header-bg)]/95 px-5 pl-18 text-[var(--betanor-header-text)] backdrop-blur lg:px-8 lg:pl-8">
    ...
    <div className="relative ml-auto flex items-center gap-1 sm:gap-2">
      ...
      {profileOpen ? <div className="absolute top-12 right-0 w-64 rounded-xl border border-[var(--betanor-border)] bg-white p-2 shadow-xl"><p className="truncate px-3 py-2 text-xs text-[var(--betanor-muted)]">{email}</p><Link href="/portal" className="flex min-h-10 items-center rounded-lg px-3 text-sm font-semibold text-[var(--betanor-navy)] hover:bg-slate-100">Open customer portal</Link><button type="button" disabled={signingOut} onClick={signOut} className="flex w-full min-h-10 items-center rounded-lg px-3 text-sm font-semibold text-[var(--betanor-navy)] hover:bg-slate-100 disabled:opacity-50">{signingOut ? "Signing out…" : "Sign out"}</button></div> : null}
    </div>
